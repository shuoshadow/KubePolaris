...
const JobTab: React.FC<JobTabProps> = ({ clusterId, onCountChange }) => {
  ...
  const columns: ColumnsType<WorkloadInfo> = [
    {
      ...
      render: (text: string, record: WorkloadInfo) => (
        <Button
          type="link"
          onClick={() => navigate(`/clusters/${clusterId}/workloads/job/${record.namespace}/${record.name}`)}
          style={{ 
            padding: 0, 
            height: 'auto',
            whiteSpace: 'normal',
      ...
      render: (record: WorkloadInfo) => (
        <Space size="small">
          <Button
            type="link"
            size="small"
            onClick={() => navigate(`/clusters/${clusterId}/workloads/create?type=Job&namespace=${record.namespace}&name=${record.name}`)}
          >
            编辑
          </Button>
          <Button
  ...
  const handleExport = () => {
    ...
    link.download = `job_${new Date().getTime()}.csv`;
    link.click();
    ...
    <div>
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <Button
          type="primary"
          icon={<PlusOutlined />}
          onClick={() => navigate(`/clusters/${clusterId}/workloads/create?type=Job`)}
        >
          创建Job
        </Button>
      </div>
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <div>
            <strong>Job名称：</strong>{scaleWorkload?.name}
          </div>
          <div>
