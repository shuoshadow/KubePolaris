...
const DaemonSetTab: React.FC<DaemonSetTabProps> = ({ clusterId, onCountChange }) => {
  ...
  const columns: ColumnsType<WorkloadInfo> = [
    {
      ...
      render: (text: string, record: WorkloadInfo) => (
        <Button
          type="link"
          onClick={() => navigate(`/clusters/${clusterId}/workloads/daemonset/${record.namespace}/${record.name}`)}
          style={{ 
            padding: 0, 
            height: 'auto',
            whiteSpace: 'normal',
      ...
      render: (record: WorkloadInfo) => (
        <Space size="small">
          <Button
            type="link"
            size="small"
            onClick={() => navigate(`/clusters/${clusterId}/workloads/create?type=DaemonSet&namespace=${record.namespace}&name=${record.name}`)}
          >
            编辑
          </Button>
          <Button
  ...
  const handleBatchRedeploy = async () => {
    if (selectedRowKeys.length === 0) {
      message.warning('请选择要重新部署的DaemonSet');
      return;
    }
  ...
  const handleExport = () => {
    ...
    link.download = `daemonset_${new Date().getTime()}.csv`;
    link.click();
    ...
    <div>
      ...
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <Button
          type="primary"
          icon={<PlusOutlined />}
          onClick={() => navigate(`/clusters/${clusterId}/workloads/create?type=DaemonSet`)}
        >
          创建DaemonSet
        </Button>
      </div>
      ...
      >
        <Space direction="vertical" style={{ width: '100%' }}>
          <div>
            <strong>DaemonSet名称：</strong>{scaleWorkload?.name}
          </div>
          <div>
